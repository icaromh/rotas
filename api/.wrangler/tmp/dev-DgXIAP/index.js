...
// .wrangler/tmp/bundle-GVJyvx/checked-fetch.js
...
// .wrangler/tmp/bundle-GVJyvx/middleware-loader.entry.ts
var import_checked_fetch76 = __toESM(require_checked_fetch());
init_modules_watch_stub();

// .wrangler/tmp/bundle-GVJyvx/middleware-insertion-facade.js
...
app.post("/api/auth/callback", async (c) => {
  ...
  try {
    ...
    const expiresAt2 = new Date(tokenData.expires_at * 1e3).toISOString();
    ...
    const { data, error } = await supabase.from("users").upsert({
      ...
      sync_status: "idle"
    }, { onConflict: "strava_athlete_id" }).select("id").single();
...
app.post("/api/sync", async (c) => {
  ...
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  await supabase.from("users").update({ sync_status: "queued", sync_progress: 0 }).eq("id", userId);
...
app.get("/api/sync/status", async (c) => {
  ...
  const supabase = createClient(c.env.SUPABASE_URL, c.env.SUPABASE_SERVICE_ROLE_KEY);
  const { data, error } = await supabase.from("users").select("sync_status, sync_progress, rate_limit_reset_at").eq("id", userId).single();
...
var src_default = {
  ...
  async queue(batch, env) {
    ...
    for (const msg of batch.messages) {
      ...
      try {
        const { data: userRecord } = await supabase.from("users").select("rate_limit_reset_at, sync_status, sync_progress, strava_access_token").eq("id", userId).single();
        ...
        if (userRecord.rate_limit_reset_at) {
          ...
          if (Date.now() < resetTime) {
            ...
            const delaySeconds = Math.ceil((resetTime - Date.now()) / 1e3);
            msg.retry({ delaySeconds });
            continue;
          }
        }
        await supabase.from("users").update({ sync_status: "syncing" }).eq("id", userId);
        const { data: latestActivity } = await supabase.from("activities").select("start_date").eq("user_id", userId).order("start_date", { ascending: false }).limit(1).single();
        let afterParam;
        if (latestActivity && latestActivity.start_date) {
          afterParam = Math.floor(new Date(latestActivity.start_date).getTime() / 1e3);
        }
        let page = 1;
        const perPage = 200;
        let hasMore = true;
        ...
        while (hasMore) {
          ...
          if (limitHeader && usageHeader) {
            ...
            if (fifteenMinUsage >= fifteenMinLimit - 10) {
              ...
              const resetAt = new Date(Date.now() + 15 * 60 * 1e3).toISOString();
          ...
          for (const activity of activities) {
            ...
            const decoded = import_polyline.default.decode(activity.map.summary_polyline);
            const geoJSONLineString = {
              type: "LineString",
              coordinates: decoded.map(([lat, lng]) => [lng, lat])
              // polyline is [lat, lng], GeoJSON is [lng, lat]
            };
